...
`build/lib/libnvecdclient.dylib`（macOS）または`libnvecdclient.so`（Linux）が生成されます。
...
int main() {
    ...
    // 接続（Expectedは成功時にtrue）
    if (auto result = client.Connect(); !result) {
        std::cerr << "接続失敗: " << result.error().message() << std::endl;
        return 1;
    }
    ...
    if (auto result = client.Vecset("item123", vec1); !result) {
        std::cerr << "Vecset 失敗: " << result.error().message() << std::endl;
        return 1;
    }
...
`find_package(nvecd CONFIG REQUIRED)`を使用し、targetを`nvecd::client`へlinkしてください。
...
if (info) {
    ...
    std::cout << "総コマンド数: " << info->total_commands_processed << "\n";
}
```
...
```c
// イベント追跡: nvecdclient_event(client, ctx, type, id, score)
...
```cpp
if (auto result = client.Connect(); !result) {
    std::cerr << "接続失敗: " << result.error().message() << "\n";
    return 1;
}
...
int main() {
    ...
    // ユーザー行動を追跡
    client.Event("user_alice", "ADD", "laptop_001", 100);  // 購入
    client.Event("user_alice", "ADD", "laptop_002", 80);   // 閲覧
    client.Event("user_bob", "ADD", "laptop_001", 100);    // 購入
    client.Event("user_bob", "ADD", "phone_001", 90);      // 閲覧
