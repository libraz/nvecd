...
### サマリ (100Kベクトル, dim=128, cosine, Apple M4 Max)

| クエリタイプ | コールド（キャッシュなし） | ウォーム（キャッシュあり） |
|---|---|---|
| SIM (ID検索, vectors) | **1.12ms** | 0.00025ms |
| SIMV (ベクトルクエリ) | **0.98ms** | 0.00025ms |

**主要パフォーマンス指標:**
- 最適化パイプラインによる**4.71倍**の累積高速化
- キャッシュヒットスループット **400万ops/秒**
...
|---|---|---|
| 1K ベクトル | 0.015ms | 0.015ms (フルスキャン) |
| 10K ベクトル | 0.13ms | 0.13ms (フルスキャン) |
| 100K ベクトル | 1.12ms | 1.12ms (フルスキャン) |
| 1M ベクトル | 約11ms | **約0.12ms** |
