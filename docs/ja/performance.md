...
カーネル演算では NEON がスカラー比およそ 5-6 倍、AVX2 は推定 10-12 倍です。
演算ごとの実測値は[ベンチマーク](./benchmarks.md#simd性能)にあり、ここでは
再掲しません。変更時に直す場所を 1 箇所にするためです。
...
- `avg_hit_latency_ms`: キャッシュルックアップ時間。実測のヒットは 208ns で、検索レイテンシに対して十分小さい
...
| 次元 | 典型的な用途 | スキャン相対コスト* |
|------|-------------|------------------|
| 128 | 高速ルックアップ、中程度の品質 | 1倍 |
| 384 | バランス型（sentence transformers） | 約3倍 |
| 768 | 高品質（BERT 埋め込み） | 約6倍 |
| 1536 | 非常に高品質（OpenAI） | 約12倍 |

\* 総当たりスキャンは O(n × d) のため、クエリ時間は次元にほぼ比例します。実測が
あるのは dim=128 のみで、10K ベクトルで 0.092ms、100K で 0.90ms（いずれも NEON）
——[ベンチマーク](benchmarks.md#検索レイテンシ)にあります。他の行はこの比例関係
から導いた値で実測ではなく、AVX2 での実測はどの次元にもありません。
...
詳細な実測ベンチマークデータは **[ベンチマーク](benchmarks.md)** ページを参照してください。

### サマリ (100Kベクトル, dim=128, cosine, Apple M5 Max)

| クエリタイプ | コールド（キャッシュなし） | ウォーム（キャッシュあり） |
|---|---|---|
| SIM (ID検索, vectors) | **0.90ms** | 0.00021ms |
| SIMV (ベクトルクエリ) | **0.91ms** | 0.00021ms |

**主要パフォーマンス指標:**
- 最適化パイプラインによる**5.86倍**の累積高速化
- キャッシュヒットスループット **480万ops/秒**
- 統合ストレージによる**50%メモリ削減**（二重map廃止）
- **書込ブロッキング0ms**（160msのCompact()再構築を解消）

### スケーリング特性

**クエリレイテンシ vs データセットサイズ（vectorsモード、dim=128、NEON）:**

| データセットサイズ | フルスキャン | サンプリング (10K) |
|---|---|---|
| 1K ベクトル | 0.010ms | 0.010ms (フルスキャン) |
| 10K ベクトル | 0.092ms | 0.092ms (フルスキャン) |
| 100K ベクトル | 0.90ms | 0.90ms (フルスキャン) |
| 1M ベクトル | 約9ms | **約0.10ms** |

**近似検索**（`sample_size: 10000`、デフォルト）により、1M+スケールでもサブミリ秒レイテンシを実現。
...
1. **SIMD 高速化**（NEON はスカラー比で dim=128 なら 5-6 倍、dim=768 なら 3-4 倍）
2. **スマートキャッシング**（繰り返しクエリが 10K で約 400 倍、100K で約 4,300 倍高速）
3. **効率的なデータ構造**（圧縮イベント、最適化レイアウト）

10K-100K ベクトルの典型的なワークロードでは：
- **コールドクエリ**: 0.09-0.90ms
- **キャッシュクエリ**: 0.21us（208ナノ秒）
