...
### クエリレイテンシ概要 (100Kベクトル, dim=128, Apple M5 Max)

| 操作 | コールドレイテンシ | キャッシュヒット | スループット/スレッド |
|---|---|---|---|
| SIM | **0.90ms** | 0.00021ms | 約1,100 QPS |
| SIMV | **0.91ms** | 0.00021ms | 約1,100 QPS |

1Mベクトル（デフォルトサンプリング10K）の[外挿値](benchmarks.md#近似検索サンプリング): クエリあたり **約0.10ms**、8スレッドで **約80K QPS**。実測は100Kベクトルまでで、そこからの外挿です。
