...
class TieredVectorStore {
 public:
   ...
   */
  utils::Expected<void, utils::Error> Add(const std::string& id,
   ...
   */
  utils::Expected<void, utils::Error> Update(const std::string& id,
                                             std::vector<float> vec);
   ...
   */
  std::vector<TieredSearchResult> Search(const float* query,
 ...
 private:
  ...
  std::vector<TieredSearchResult> SearchMain(const float* query,
                                             uint32_t top_k) const;
  std::vector<TieredSearchResult> SearchDelta(const float* query,
                                              uint32_t top_k) const;
  static std::vector<TieredSearchResult> MergeResults(
