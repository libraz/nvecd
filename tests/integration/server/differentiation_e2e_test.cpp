...
TEST_F(DifferentiationDisabledE2ETest, TimestampParameter_E2E) {
  ...
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT user1 ADD item1 10 timestamp=1000")));
  ...
  // SET with timestamp
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT user1 SET item3 5 timestamp=2000")));

  // DEL with timestamp
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT user1 DEL item1 timestamp=3000")));

  // Invalid timestamp should error
  EXPECT_TRUE(ContainsError(
      client.SendCommand("EVENT user1 ADD item4 10 timestamp=abc")));
}
...
TEST_F(DifferentiationE2ETest, TemporalCooccurrence_E2E) {
  ...
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD old_a 10 timestamp=1000")));
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD old_b 10 timestamp=1000")));

  // Add recent events (timestamp=9000)
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD new_c 10 timestamp=9000")));
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD new_d 10 timestamp=9000")));
  ...
  if (!results.empty()) {
    ...
    EXPECT_TRUE(found_new_d)
        << "new_d should appear in co-occurrence results for new_c";
  }
}
...
TEST_F(DifferentiationE2ETest, NegativeSignals_E2E) {
  ...
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD item_x 10 timestamp=5000")));
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD item_y 10 timestamp=5000")));
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD item_z 10 timestamp=5000")));
  ...
  EXPECT_GT(score_x_before, 0.0F)
      << "item_x should appear in results before DEL with positive score";
  ...
  for (const auto& [id, score] : results_after) {
    if (id == "item_x") score_x_after = score;
    if (id == "item_z") score_z_after = score;
  }

  // item_x's score should be lower than before (penalized by negative signal)
  EXPECT_LT(score_x_after, score_x_before)
      << "item_x score should decrease after DEL (negative signal)";

  // item_z should still have a positive score (unaffected)
  EXPECT_GT(score_z_after, 0.0F)
      << "item_z should still have positive co-occurrence";
}
...
TEST_F(DifferentiationE2ETest, AdaptiveFusion_E2E) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    EXPECT_TRUE(ContainsOK(client.SendCommand(
        "EVENT ctx" + std::to_string(i) +
        " ADD mature_item 10 timestamp=5000")));
    EXPECT_TRUE(ContainsOK(client.SendCommand(
        "EVENT ctx" + std::to_string(i) + " ADD " + item +
        " 10 timestamp=5000")));
  }
  ...
  // Add vectors
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("VECSET mature_item 1.0 0.0 0.0")));
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("VECSET new_item 0.9 0.1 0.0")));
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("VECSET partner_0 0.8 0.2 0.0")));
...
TEST_F(DifferentiationE2ETest, AdaptivePerQuery_E2E) {
  ...
  // Setup data
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD item1 10 timestamp=5000")));
  EXPECT_TRUE(ContainsOK(
      client.SendCommand("EVENT user1 ADD item2 10 timestamp=5000")));
  ...
  auto resp_on =
      client.SendCommand("SIM item1 10 using=fusion adaptive=on");
  EXPECT_TRUE(ContainsOK(resp_on));

  // Query with adaptive=off (uses static weights)
  auto resp_off =
      client.SendCommand("SIM item1 10 using=fusion adaptive=off");
...
TEST_F(DifferentiationE2ETest, AllFeaturesCombined_E2E) {
  ...
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT u1 ADD a 10 timestamp=1000")));
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT u1 ADD b 10 timestamp=1000")));
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT u1 ADD c 10 timestamp=9000")));
  EXPECT_TRUE(
      ContainsOK(client.SendCommand("EVENT u1 ADD d 10 timestamp=9000")));
